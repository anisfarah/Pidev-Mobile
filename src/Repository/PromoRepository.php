...
class PromoRepository extends ServiceEntityRepository
{
    ...
    public function search($query)
    {
        ...
        $qb->where($qb->expr()->like('promo.reduction', ':query'))
        ->setParameter('query', '%'.$query.'%');
        return $qb->getQuery()->getResult();
    }

    public function findAllOrderByReductionDESC()
    {
        return $this->createQueryBuilder('promo')
            ->orderBy('promo.reduction', 'DESC')
            ->getQuery()
            ->getResult();
    }

    public function findAllOrderByReductionASC()
    {
        return $this->createQueryBuilder('promo')
            ->orderBy('promo.reduction')
            ->getQuery()
            ->getResult();
    }
